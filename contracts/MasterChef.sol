...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    DraugrToken public draugr;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public draugrPerBlock;
    // Bonus muliplier for early draugr makers.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(draugrReward.mul(1e12).div(lpSupply));
        }
    ...
    // Safe draugr transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 draugrBal = draugr.balanceOf(address(this));
        if (_amount > draugrBal) {
            draugr.transfer(_to, draugrBal);
        } else {
            draugr.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _draugrPerBlock) public onlyOwner {
        massUpdatePools();
        draugrPerBlock = _draugrPerBlock;
    }
}
